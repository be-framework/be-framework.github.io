...
一度定義すれば、`$email`という名前のすべてのコンストラクタ引数に自動的に適用されます。`$email`の値は常に正しいことが保証されます。正しくない値は存在できません。

## 属性による制約の拡張

同じ名前に属性を加えることで、存在条件をより精密にできます。`#[Validate]`属性が付いたメソッドの引数にPHP属性がある場合、コンストラクタ引数に同じ属性があるときだけ実行されます：
